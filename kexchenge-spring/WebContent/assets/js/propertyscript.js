...
$(document).ready(function(){  
  ...
  $("#updatePropertyForm").validationEngine();     
        ...
        //extra valus in add property
          $.fn.valList = function(){
            ...
            $("#add_pro_cost").blur(function(){
              if($(this).val()=='')
              {
                $(this).removeAttr("class");
                $(this).addClass("validate[custom[integer],maxSize[9]]");
                $("#add_pro_rate").removeAttr("class");
                $("#add_pro_rate").addClass("validate[required,custom[integer],maxSize[7]]");
              }else{
                 $("#add_pro_rate").removeAttr("class");
                $("#add_pro_rate").addClass("validate[custom[integer],maxSize[7]]");
              }
         ...
         $('#ter_add_val').click(function(){
                ...
                $('#ter_val').slideToggle();
                    ...
                    $('#optional_info span#ter_extra').remove();
                  }
           });
          $('#ter_val').blur(function(){
              var $val=$(this).val();

              $('#optional_info').append("<span id='ter_extra'>"+$val+"</span>");
                   $('#ter_add_val').val($val);
               
           });

 
           $("#sidebyside_mess div span").mouseenter(function(){
                  var val = $(this).attr('title');
                  $(this).after('<label class="tooltip">'+val+'</label>');
          
                }).mouseout(function(){
                  $("#sidebyside_mess label.tooltip").hide();
                });
           ...
           $('#add_av_val').click(function(){
                ...
                $('#av_val').slideToggle();
                    ...
                    $('#optional_info span#avg_extra').remove();
                  }
           });
          ...
          $('#av_val').blur(function(){
                ...
                $val='AV'+$(this).val();
                $('#optional_info').append("<span id='avg_extra'>"+$val+"</span>");
                $('#add_av_val').val($val);
           ...
           $("#propertyForm input[type='checkbox'][name='opv[]']").click(function(){
            ...
            $("#updatePropertyForm input[type='checkbox'][name='opv[]']").click(function(){
              $val=$(this).val();
              if(this.checked){
              $('#optional_info').append("<span id="+$val+">"+$val+"</span>");
            }else{
          ...
          $("#propertyForm :input[name='property_type_base']").change(function(){
           $type=$(this).val();    
           ...
           $("#updatePropertyForm :input[name='property_type_base']").change(function(){
           $type=$(this).val();    
                $.post("index.php?action=property_type_by_ajax",{type:$type},function(data,status){
                      $("#add_pro_property_type").html(data);
          ...
          $("#add_pro_location_city").change(function(){
           ...
           $city=$(this).val();
           if($city!='o'){ $("#add_pro_city").fadeOut();
          ...
          $("#add_pro_location_town").change(function(){
            var $msgBox = $('.error-show');
           $town=$(this).val();
           if($town!='o'){  $("#add_pro_town").fadeOut();
          ...
          $("#add_pro_location_location").change(function(){
            var $msgBox = $('.error-show');
           $town=$(this).val();
           if($town!='o'){$("#add_pro_location").fadeOut();
                if($town=='no')
                {
            ...
            var $msgBox = $('.error-show');
           $town=$(this).val();
           if($town!='o'){$("#add_pro_type").fadeOut();
                if($town=='no')
                {
            ...
            var $msgBox = $('.error-show');
           $town=$(this).val();
           if($town!='o'){$("#add_pro_floor_other").fadeOut();
                if($town=='no')
                {
 ...
 //property post  validation
              ...
              {
                var $city = $('#add_pro_location_city').val();
                var $pti= $("#property_type_base").val();
                var $town = $("#add_pro_location_town").val();
                var $location = $("#add_pro_location_location").val();
                ...
                var $depo = $("#add_pro_depo").val();
                 ...
                 if($sellrent==undefined){
                    ...
                    $msg.html("<font color='red'>Select Sell or Rent!</font>");
                    return false;
                  }else if(($pti=='')){
                      $("#property_type_base").focus();
                        $msg.html("<font color='red'>Select residential or commercial!</font>");
                        return false;
                  }else if($city=='no'){
                    ...
                    return false;
                      ...
                      $("#add_pro_city").focus();
                        ...
                        return false;
                  }else if(($fcity.length >=20)&&($city==0)){
                      
                      $("#add_pro_city").focus();
                        $msg.html("<font color='red'>Enter between 1-20 characters in city name!</font>");
                        return false;
                  }else if($town=='no'){
                    ...
                    return false;
                      ...
                      $("#add_pro_town").focus();
                        ...
                        return false;
                  }else if(($ftown.length >=20)&&($town==0)){
                      $("#add_pro_town").focus();
                        $msg.html("<font color='red'>Enter between 1-20 characters in town name!</font>");
                        return false;
                  }else if($location=='no'){
                    ...
                    return false;
                      ...
                      $("#add_pro_location").focus();
                        ...
                        return false;
                  }else if(($flocation.length >=20)&&($location==0)){
                      $("#add_pro_location").focus();
                        $msg.html("<font color='red'>Enter between 1-20 characters in location name!</font>");
                        return false;
